...
::ActiveSupport::Dependencies.load_paths << Rails.root.join("app", "models", "posts")
::ActiveSupport::Dependencies.load_paths << Rails.root.join("app", "models", "projects")

module Alfred

  def self.config
    @config ||= YAML.load_file(File.expand_path("config/alfred.yml", Rails.root))
  end
